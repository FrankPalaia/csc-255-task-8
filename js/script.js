...
function swapCase(string) {
    var result="";
    var swapString = string.split("");
    for (var i=0; i < swapString.length; i++){
        if (swapString[i]==swapString[i].toLowerCase()){
            swapString[i]=swapString[i].toUpperCase();
    }
    else{
        swapString[i]=swapString[i].toLowerCase();
    }
    result+=swapString[i];
    }
...
//convert array of numbers from farenheit to celsius
// example:
// argument: [23, 32, 41, 50, 59]
// return: [-5, 0, 5, 10, 15]
// hint: use Array.map
function toCelsius(array){
const celsius = array.map(function(n){
    return n-32;
});
return celsius;
}
...
function germanNumbers(){
    ...
    cardinalNumbers.forEach((i, j) => {
        const translation = germanNumbers[j];
        console.log(i + " is " + translation);
      });
}
...
function returnPrimeNumbers(){
    ...
    var result = numbers.filter(function(n) {
        return (n>1 && n%2>0 && n%3>0) || n==2 || n==3;
    });
    return result;  
}
